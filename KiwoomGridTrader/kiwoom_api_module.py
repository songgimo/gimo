...
class KiwoomApiModule:
    ...
    def _send_order(self, rq_name, scn_no, account, order_type, stock_code, qty, price, trade_type, ogn_order_no=''):
        debugger.debug('order - {}'.format([rq_name, scn_no, account, order_type, stock_code, qty, price, trade_type,
                                    ogn_order_no]))
        # 매매 주문을 하는 함수
        '''
              SendOrder(
        ...
        self.kh_regist.dynamicCall('SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)',
                                   [rq_name, scn_no, account, order_type, stock_code, qty, price, trade_type,
                                    ogn_order_no])
        debugger.debug('order DONE- {}'.format([rq_name, scn_no, account, order_type, stock_code, qty, price, trade_type,
                                            ogn_order_no]))

    def _set_real_reg(self, scr_no, stock_code, fid, regist_type):
        '''

        :param scr_no: 화면 번호
        :param stock_code: 종목코드
        :param fid: 실시간 FID리스트 ( KOA 참조 )
        :param regist_type: 실시간 등록 타입
            0: 등록된 종목을 실시간 해제하고 새로 등록한 종목만 실시간 조회
            1: 먼저 등록한 종목을 포함해 실시간 시세 등록 조회
        '''

        return self.kh_regist.dynamicCall('SetRealReg(QString, QString, QString, QString)',
                                   [scr_no, stock_code, fid, regist_type])

    def get_real_current_price(self, stock_code, fid, regist_type, scr_no=None):
        '''
        :param stock_code: 실시간 조회를 할 종목코드
        :param fid: FID ( KOA 참조 )
        :param regist_type: 실시간 등록 타입 ( _set_real_reg함수 참조 )
        :param scr_no: 화면 번호, 일반적으로는 종목 코드를 따른다.
        '''
        if scr_no is None:
            scr_no = stock_code

        return self._set_real_reg(scr_no, stock_code, fid, regist_type)
    ...
    def get_common_real_data(self, stock_code, field):
        # self._set_real_reg(stock_code, str(field))
            return self.kh_regist.dynamicCall('GetCommRealData(QSting, QString)', [stock_code, field])

    def disconnect_real_data(self, scn_no):
        return self.kh_regist.dynamicCall('DisconnectRealData(QString)', scn_no)
